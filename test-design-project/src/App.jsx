import Header from "./layout/Header"
import Content from "./layout/Content"
import Footer from "./layout/Footer"
import "./app.css"
import React from "react"

function App() {
  
  class MyComponent extends React.Component{
    constructor(props){
      super(props);
      this.state = {favouritecolor: "blue", favouritecolor2: "yello"}
    }
    static getDerivedStateFromProps(props, state){
      if (props.favcol == null || props.favcol2 == null) {
        return;
      }else{
        return {favouritecolor: props.favcol, favouritecolor2: props.favcol2};
      }
      return {favouritecolor: props.favcol, favouritecolor2: props.favcol2};
    }
    render(){
      return(
      <div>
        <h1>my fav color is {this.state.favouritecolor} and {this.state.favouritecolor2}</h1>
      </div>
      )
    }
  }

  return(
    <div className="app">
      <img src="https://proxy.notsobot.com/google-images?url=https%3A%2F%2Fimages-wixmp-ed30a86b8c4ca887773594c2.wixmp.com%2Ff%2F053bc983-4e4f-474f-bb0e-687f077d7067%2Fdg1vwtt-320b4c53-2683-41d0-b78f-5845c52be3bb.jpg%3Ftoken%3DeyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..aCzZz6Qe-z6_y9ZuD_wBtGZQiW2GN0b0S_Wt3f3eFxM&backup=https%3A%2F%2Fencrypted-tbn0.gstatic.com%2Fimages%3Fq%3Dtbn%3AANd9GcRBk_FnSnKdpWc_Qds1BpWMItoT6Jrt5C6Ro8FV_0-dNsFInGUFvUyg05gh0QWlxiw0vQ4%26usqp%3DCAU" alt="" />
      <Header></Header>
      <Content></Content>
      <Footer></Footer>
      <MyComponent favcol="wow" favcol2="wee"/>
    </div>
  )
}

export default App
